...
## Pesquisa: disciplinas de tecnologia nos cursos de Letras

Ferramenta separada (pasta `letras/`) para levantar disciplinas sobre ensino de tecnologias nos
cursos de Letras das universidades públicas brasileiras (federais + estaduais). Ela busca os
projetos pedagógicos (PPC) e matrizes curriculares na web, baixa os PDFs/páginas, extrai o texto e
identifica as disciplinas relacionadas a tecnologia.

```bash
# 1. (opcional) atualizar a lista de universidades a partir da Wikipédia
.venv/bin/python scripts/atualizar_universidades.py

# 2. testar os documentos encontrados, sem baixar
.venv/bin/python scripts/coletar_letras.py --sigla UFRJ --so-listar

# 3. coletar uma ou mais universidades (por sigla, UF ou categoria)
.venv/bin/python scripts/coletar_letras.py --sigla UFRJ UNB UFMG --max-docs 8
.venv/bin/python scripts/coletar_letras.py --uf RJ
.venv/bin/python scripts/coletar_letras.py --categoria estadual
.venv/bin/python scripts/coletar_letras.py --reparse   # re-extrai sem baixar de novo
...
automaticamente o **Wayback Machine** (PDF original) e, se necessário, o **r.jina.ai** (texto).

O resultado fica em `data/letras/letras.db` (SQLite). Para gerar a planilha:

```bash
.venv/bin/python scripts/exportar_letras.py --formato csv   # data/letras/disciplinas.csv
.venv/bin/python scripts/exportar_letras.py --universidade UFRJ --formato json
```

Arquivos e configuração do coletor: `letras/` (`search.py`, `fetch.py`, `extract.py`, `parse.py`,
`store.py`). Ajuste tempos de espera e limites por variáveis de ambiente (`LETRAS_*`) — veja
`letras/config.py`. A lista base de instituições é `data/universidades_publicas.csv`.

> A coleta usa busca web (pacote `ddgs`) e heurísticas de texto. Os resultados são **candidatos**
> e devem ser conferidos no PPC oficial antes de uso acadêmico; PPCs digitalizados (imagem) podem
> não ter texto extraível.
